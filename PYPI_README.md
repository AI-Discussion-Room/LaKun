...
灵感来源于JEV，感谢laya https://github.com/mizorewww/laya-mlx 提供的优化方式，所以取名文LaJ(谐音垃圾)，因致敬前辈故LaKun诞生
LaKun是决策类型模型的多模态版本
面对一段文本状态或一张图片，它直接回答我指定的选择、档位评分和二元判断问题，
输出每个候选项的softmax分数与最高分选项，不生成自由文本。一次调用最多可提交20道问题，目前每次最多处理一张图片。
这个PyPI安装包提供推理代码，源码包另含训练脚本；两种发行文件都**不包含权重或数据集**。我的完整检查点有 756,409,158 个参数；`lakun.safetensors` 大小为 3,025,715,432 字节（2.82 GiB），完整推理目录约 2.85 GiB。`0.7B` 是模型仓库名中的近似称呼。
